...
from cube import *
class hhh():
    def __init__(self,r,h,beta,x,y,z,work):
        #("begin")
        xlsbpath = r"./"
        ...
        def p_solve():
            ...
            for height in range(h + 1 - t):
                for j in range(a + 1):
                    for l in range(a + 1):
                        point_a = Point(l / a - 0.5+x, j / a - 0.5+y, height+z,p_num)
                        p_num=p_num+1
                ...
                for angle in range(beta):
                    for radius in range(2, r + 1):
                        point_a = Point(radius * math.cos((angle / beta - 3 / 8) * 2 * math.pi)+x,
                                        radius * math.sin((angle / beta - 3 / 8) * 2 * math.pi)+y, height+z,p_num)
                        p_num=p_num+1
                        file.write(str(point_a.x) + ' ' + str(point_a.y) + ' ' + str(point_a.z) + '\n')
                        self.p.append(point_a)
                        radius = radius + 1
            ...
            for height in range(h + 1 - t, h + 1):
                for j in range(a + 1):
                    for l in range(a + 1):
                        point_a = Point((l / a - 0.5) * math.pow(1 - (height - h + t) * (height - h + t) / t / t, 0.5)+x,
                                        (j / a - 0.5) * math.pow(1 - (height - h + t) * (height - h + t) / t / t, 0.5)+y,
                                        height+z,p_num)
                        p_num=p_num+1
                        file.write(str(point_a.x) + ' ' + str(point_a.y) + ' ' + str(point_a.z) + '\n')
                        self.p.append(point_a)
